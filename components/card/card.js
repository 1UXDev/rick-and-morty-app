/* import { SearchBar, SearchQuery } from "../search-bar/search-bar.js";
SearchBar();

console.log("hello im logging");

export function hiFunct() {
  console.log("hi");
}

export async function createCharacterCard(
  _navigate_number = 1,
  SearchQuery = ""
) {
  const cardContainer = document.querySelector('[data-js="card-container"]');
  const card__image = document.querySelector('[data-js="card__image"]');
  const card__title = document.querySelector('[data-js="card__title"]');
  const card__info_status = document.querySelector(
    '[data-js="card__info_status"]'
  );
  const card__info_type = document.querySelector('[data-js="card__info_type"]');
  const card__info_Occurrences = document.querySelector(
    '[data-js="card__info_Occurrences"]'
  );
  const searchBarForm = document.querySelector(`[data-js="search-bar"]`);
  const searchbarvalue = searchBarForm.firstChild.nextSibling.value;

  const response = await fetch(
    "https://rickandmortyapi.com/api/character/" + _navigate_number
  );
  const jsonData = await response.json();

  async function myFunction() {
    const ObjectData = await fetch(
      `https://rickandmortyapi.com/api/character/`
    );
    const JSONObject = await ObjectData.json();

    //   if(SearchQuery !== "")
    //     {
    //     for (i = 1; i <= 10; i++) {
    //       const response_array = await fetch(`https://rickandmortyapi.com/api/character/${[_navigate_number]}`);
    //       const jsonData_array = await response_array.json();
    //       dataBase.push(i, jsonData_array.name);
    //     }
    //     result = dataBase.filter((img) => (img.name).startsWith(SearchQuery)).first();
    //     jsonData =result ;
    //     response = await fetch("https://rickandmortyapi.com/api/character/"+ jsonData[0]);
    //     jsonData =  await response.json();
    //     console.log(jsonData)
    //   }
    //   console.log(jsonData)
  }

  card__title.innerHTML = jsonData.name;
  card__info_status.innerHTML = jsonData.status;
  card__info_type.innerHTML = jsonData.type;
  card__info_Occurrences.innerHTML = jsonData.species;
  card__image.setAttribute(
    "src",
    "https://rickandmortyapi.com/api/character/avatar/" +
      _navigate_number +
      ".jpeg"
  );
  card__image.setAttribute("alt", "Rick Sffsanchez");
  card__image.setAttribute("data-js", "card__image");
  card__image.setAttribute("class", "card__image");
}

SearchBar();
console.log(SearchQuery);

export async function myFunction(value) {
  const ObjectData = await fetch(`https://rickandmortyapi.com/api/character/`);
  const JSONObject = await ObjectData.json();

  console.log(value);

  JSONObject.results.forEach((object) => {
    if (object.name.toLowerCase().includes(value.toLowerCase())) {
      console.log(object.name);
    }
  });
}
 */
